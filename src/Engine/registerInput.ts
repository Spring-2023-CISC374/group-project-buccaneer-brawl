...
export default class registerInput extends Phaser.Scene {
  ...
  public validInput(
    ...
    if (
      player?.action === 'attack/punch' ||
      input[this.key_index] === 'attack/kick' ||
      input[this.key_index] === 'attack/hook'
    )
      nextMoveTime = 250;
    ...
    while (this.key_timer > nextMoveTime) {
      ...
      if (
        input[this.key_index].startsWith('walk') ||
        input[this.key_index].startsWith('jump')
      ) {
